...
class MyKmeans:
    def __init__(self, k=2, tol=0.01, max_rep=100, init_type='random'):
        self.k = k
        self.tol = tol
    ...
    '''
    def predict(self, df):
        dist2centroids = [self.find_mindist(df, seed) for seed in self.centroids]
        # dist2centroids has k columns which correspond to the dist from each centroid
        dist_df = pd.concat(dist2centroids, axis=1).idxmin(axis=1)

        for seed in self.centroids:
            if (dist_df == seed).values[0]:
                return seed
    '''
